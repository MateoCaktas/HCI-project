...
class FilteredProducts extends Component {
  ...
  render() {
    ...
    const matchedPosts = searchResults.map(({ markdown }) => {
      const { id } = markdown;
      const { title, slug, image, zanr, ocjena } = markdown.frontmatter;
        ...
        <div key={id} className={styles.PostContainer}>
          <Link to={`blog/${slug}`} className={styles.Link}>
            ...
            <div className="Post-text">
              <div className="Postzanra">Genre:</div> 
              <div className="Post-ocjena">Rating:</div>
            </div>
            <div className="Post-text">
              <div className="Postzanra">{zanr}</div> 
              <div className="Post-ocjena">{ocjena}</div>
            </div>
          </Link>
        </div>
